...
from configurations import config1 as config
...
metadata_target_path = 'metadata/%s' % experiment_id
...
tunes = tune_parser.loadTrainingSet()
tunes = [tune.getTrainingTokens() for tune in tunes]
...
tunes = [[token2idx[c] for c in [start_symbol] + t + [end_symbol]] for t in tunes]
...
nvalid_tunes=long(nvalid_tunes)
...
for epoch in xrange(start_epoch, config.max_epoch):
    for train_batch_idxs in train_data_iterator:
        ...
        training_loss_list.append(float(train_loss))
    ...
    if (epoch + 1) % config.save_every == 0:
        with open(metadata_target_path+".pkl", 'wb') as f:
            pickle.dump({
        ...
        pickle.dump(training_loss_list,open(metadata_target_path+"_loss.pkl","wb"),pickle.HIGHEST_PROTOCOL)
